    ...
    <div>
      ...
      <div className="card" style={{ marginBottom: 20 }}>
        <div className="tab-bar">
          ...
          >📑 议题切分 ({meeting.topics.length})</button>
          <button
            className={`tab-item ${activeTab === 'questions' ? 'active' : ''}`}
            onClick={() => setActiveTab('questions')}
          >❓ 客户问题 ({meeting.questions.length})</button>
          <button
            className={`tab-item ${activeTab === 'followups' ? 'active' : ''}`}
            onClick={() => setActiveTab('followups')}
          >📋 跟进事项 ({meeting.followUps.length})</button>
          <button
            className={`tab-item ${activeTab === 'checklist' ? 'active' : ''}`}
            onClick={() => setActiveTab('checklist')}
          >✅ 复盘清单 ({completedCount}/{REVIEW_CHECKLIST.length})</button>
          <div style={{ flex: 1 }} />
            ...
            {meeting.questions.map(q => (
              <div key={q.id} className="question-item">
                ...
                <div className="question-meta">
                  ...
                  <span style={{ color: '#94a3b8', fontSize: 12 }}>出现在 {formatDuration(meeting.transcripts.find(t => t.id === q.segmentId)?.startTime || 0)}</span>
                </div>
              </div>
            ))}
              ...
              return (
                <div key={f.id} className="followup-item">
                  ...
                  <div className="followup-body">
                    <div className="followup-content" style={{ textDecoration: f.status === 'completed' ? 'line-through' : 'none', opacity: f.status === 'completed' ? 0.6 : 1 }}>
                      {f.content}
                    </div>
